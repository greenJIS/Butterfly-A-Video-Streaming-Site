...
<div class="whole-page">
  ...
  <div class="content content-group">
	...
	<div class="videos">
  	<?php
  		$query="SELECT * FROM `video` ORDER BY `likes` DESC LIMIT 7";
  		if($query_run = mysql_query($query))
  		{
  			while($query_row=mysql_fetch_assoc($query_run))
  			{
  				...
  				/*$ffmpeg= "C:\\ffmpeg\\bin\\ffmpeg";//Path to your FFMPEG
                  $file = $link; // Path to your Video
                   ...
                   $time =  exec("$ffmpeg -i $file 2>&1 | grep 'Duration' | cut -d ' ' -f 4 | sed s/,//");   
                    ...
                    $real_minutes = floor($minutes);
  					...
  					$video_Length=$real_minutes.':'.$real_seconds;
  					
  					<p><span style=\"font-weight: bold;\">Duration : </span>$video_Length</p>
					like form
					<form action=\"likes.php\" method=\"POST\">
									<input type=\"hidden\" name=\"like\" value=\"1\">
									<input type=\"submit\" style=\"background-color: transparent; border: none; font-size: 16px; color: red; font-weight: bold;\" name=\"submit\" value=\"Like\">
								</form>
