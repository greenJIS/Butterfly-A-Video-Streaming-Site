...
<div class="content content-group">
    ...
    <div class="videos" style="font-size:20px; background:#AAAAAA"><br>
		<div>
        <fieldset style="width:50%; position:relative; left: 20%; background:#DDDDDD">
        <?php
			...
			while($query_row=mysql_fetch_assoc($query_run))
			{
				 ...
				 $name=$query_row['profile_name'];
				echo "<label for=\"user-profile\"><h2>User Information</h2></label>

				<label for=\"email\" ><span style=\"font-size:14px;color:gray;\">Email:</span><br>$email</label><br>

				<label for=\"user-id\" ><span style=\"font-size:14px;color:gray;\">User name:</span><br>$name</label><br><br>";
				echo "<br><br><hr>";
				echo "<label style=\"color:red;\">**Your account information is always classified</label>";
			}
		?>
        </fieldset>
		</div>
		...
		<div>
		<?php
			$query="SELECT * FROM `video` WHERE `video_owner`= '$id' ORDER BY `date` DESC";
			if($query_run = mysql_query($query))
			{   
				if(mysql_num_rows($query_run)!=NULL)
				{
					echo "<p style=\"text-align:center;\">Uploaded Videos</p><br>";
					while($query_row=mysql_fetch_assoc($query_run))
  			    {
				...
				{
					echo "<p style=\"text-align:center;\">You don't Upload any video</p><br>";
				}
